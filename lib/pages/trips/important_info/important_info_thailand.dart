...
const String gCurrencyBody =
    "Waluta Tajlandii to Bat (THB). Nie ma możliwości wymiany polskiego złotego na Baty, dlatego z Polski najlepiej wziąć ze sobą dolary lub Euro i te wymienić w Tajlandii. 1 USD = ok. 36 THB, a 1 EUR = ok. 41 BHT.";

const String gATMTitle = "Bankomaty";
const String gATMBody =
    "Przy każdej wypłacie gotówki z bankomatu zostanie potrącona prowizja w wysokości 200-250 THB. Tej opłaty niestety nie da się uniknąć, dlatego warto jednorazowo wyciągać większe kwoty. Uwaga! Wiele osób jest przekonanych, że uniknie płacenia powyższej prowizji, skoro ma w swoim banku „darmowe wypłaty z bankomatów na całym świecie”. Owszem, jeśli jest włączona taką opcję, to bank nie pobierze (polskiej) prowizji za pobranie pieniędzy, ale nie pokrywa kosztów prowizji tajlandzkiego operatora bankomatu.";

const String gShoppingTitle = "Zakupy";
const String gShoppingBody =
    "W Tajlandii cenę towaru określa zwykle sprzedawca i turysta z Europy musi liczyć się z tym, że zapłaci nieco więcej, niż miejscowy nabywca. Produkty tajskie nie są drogie, zwłaszcza świeże owoce, warzywa, ryby i mięso, natomiast za towary, które są sprowadzane z Europy, trzeba zapłacić znacznie więcej.";

const String gTimeTitle = "Czas";
const String gTimeBody =
    "W Tajlandii czas różni się od czasu środkowoeuropejskiego o 5 godzin w lecie oraz 6 godzin podczas Polskiej zimy.";

const String gElectricityTitle = "Elektryczność";
const String gElectricityBody =
    "Napięcie w gniazdkach wynosi 220 V. Większość gniazdek nadaje się zarówno do wtyczek z okrągłymi bolcami jak i płaskimi. Dzięki temu zazwyczaj nie są potrzebne adaptery.";

const String gCultureTitle = "Kultura";
const String gCultureBody =
    """Na co zwrócić szczególną uwagę? Na postać króla! Tajowie z ogromnym szacunkiem odnoszą się do rodziny królewskiej. Wokół króla zbudowano potężny kult jednostki. Król Tajlandii umarł 13 października 2016r. w Bangkoku, był najdłużej panującym monarchą na świecie. Przez minione 70 lat król stał się najwyższym autorytetem moralnym i ucieleśnieniem wszystkiego, co tajskie. Osiągnął on pozycję, jaką nie może poszczycić się żaden monarcha konstytucyjny na świecie. Był półbogiem, nie figurantem, symbolem zamierzchłej tradycji jak monarchowie Wielkiej Brytanii czy Holandii. Dlatego w kraju ogłoszono roczną żałobę dla urzędników państwowych, ale zwykli obywatele także pogrążyli się w smutku. Prosimy więc zwrócić szczególną uwagę na tajską kulturę i przestrzeganie zasad zachowania. Sytuację konfliktową może spowodować każde, nawet nieumyślne zlekceważenie majestatu króla Tajlandii, jego rodziny i dworu królewskiego (np. publiczne zniszczenie banknotu, na którym znajduje się wizerunek władcy).
...
const String gReligionBody =
    "Dominująca religia w Tajlandii to buddyzm. Jak wskazują źródła, około 92% całej populacji to wyznawcy właśnie tej religii. 5% mieszkańców to z kolei muzułmanie. Pozostały procent, stanowią: chrześcijanie, hinduiści oraz sikhowie.";
...
const String gSouvenirsBody =
    "Warto przywieźć z Tajlandii oryginalne wyroby rzemieślnicze oraz rękodzielnicze, które będą stanowiły cenną pamiątkę z pobytu w tym niezwykłym kraju. Tajskie stragany są pełne przeróżnych ciekawych produktów.";

const String gForbiddenSouvenirsTitle = "Zabronione pamiątki";
const String gForbiddenSouvenirsBody =
    "Zabroniony jest wywóz statuetek oraz wizerunków Buddy, na których wywóz musimy mieć pozwolenie Ministerstwa Sztuki. Przepis ten nie dotyczy małych figurek noszonych przy sobie.";

const String gInsuranceTitle = "Ubezpieczenie";
const String gInsuranceBody =
    "Są Państwo ubezpieczeni w Signal Iduna standardowym ubezpieczeniem nie obejmującym chorób przewlekłych. Ubezpieczenie nie obowiązuje w momencie, gdy klient jest pod wpływem alkoholu lub/i narkotyków - wtedy nie ma zwrotu z polisy.";
...
class ImportantInfoThailandWidget extends ImportantInfoWidget {
  List<Widget> fBuildListOfItemsTile(List<String> items) {
    return List<Widget>.from(items.map((item) => fBuildInsuranceRow(item)));
  }
  ...
  List<Widget> fBuildAirConnection(
      List<String> flights, String airLine, List<String> luggage,
      [bool divider = true]) {
    List<Widget> items = [];
  ...
  List<Widget> fBuildAirConnectionsTile(int groupIndex) {
    List<Widget> items = [];
    items.addAll(fBuildAirConnection(gAirConnectionBody[groupIndex],
        gAirLineBody[groupIndex], gTravelLuggageBody));
  ...
  Column fBuildInfoTiles() {
    ...
    fAddExpansionTileToList(
        gAirConnectionTitle, fBuildAirConnectionsTile(groupIndex));
    ...
    fAddExpansionTileToList(
        gCurrencyTitle, fBuildSimpleTextTile(gCurrencyBody));
    fAddExpansionTileToList(gATMTitle, fBuildSimpleTextTile(gATMBody));
    fAddExpansionTileToList(
        gShoppingTitle, fBuildSimpleTextTile(gShoppingBody));
    fAddExpansionTileToList(gTimeTitle, fBuildSimpleTextTile(gTimeBody));
    fAddExpansionTileToList(
        gElectricityTitle, fBuildSimpleTextTile(gElectricityBody));
    fAddExpansionTileToList(gCultureTitle, fBuildSimpleTextTile(gCultureBody));
    fAddExpansionTileToList(
        gReligionTitle, fBuildSimpleTextTile(gReligionBody));
    ...
    fAddExpansionTileToList(
        gSouvenirsTitle, fBuildSimpleTextTile(gSouvenirsBody));
    fAddExpansionTileToList(gForbiddenSouvenirsTitle,
        fBuildSimpleTextTile(gForbiddenSouvenirsBody));
    fAddExpansionTileToList(
        gInsuranceTitle, fBuildSimpleTextTile(gInsuranceBody));
